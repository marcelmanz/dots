...
refresh_volume_state() {
	local sink_info
	sink_info=$(pactl list sinks | grep -A15 "$(pactl get-default-sink)" | grep -E "(Volume:|Mute:)")

	local volume_percent
	volume_percent=$(echo "$sink_info" | grep "Volume:" | head -1 | awk '{print $5}' | tr -d '%')
	...
	local mute_output
	mute_output=$(echo "$sink_info" | grep "Mute:" | head -1 | awk '{print $2}')
